...
clf = pd.read_pickle('saved_model.sav') #replace with trained model.sav from notebooks
...
with open("te_ToNIoT_RF.py", "w") as entries_file: # file to save entries in

    print("p4 = bfrt.attack_detection.pipe\n", file=entries_file) # replace "henna" with p4 filename
...
for qsfp_cage in [1, 5]: #replace with your own switch ports
...
"""
    ...
    for num_feat in range(len(feature_names)):
        tbl_name = "tbl_s1_f" + str(num_feat)
        print(tbl_name + " = p4.Ingress."+tbl_name, file=entries_file)
    print('', file=entries_file)

    for num_tree in range(len(clf.estimators_)):
        code_tbl_name = "tbl_s1_cw"+str(num_tree)
        print(code_tbl_name+" = p4.Ingress."+code_tbl_name, file=entries_file)
    print('', file=entries_file)
    ...
    for fea in range(0,len(feature_names)):
        ...
        for ran, cods0, cods1, cods2 in zip(Ranges, Codes.iloc[:,0], Codes.iloc[:,1], Codes.iloc[:,2]):
            tbl_name = "tbl_s1_f" + str(fea)
            fea_name = "s1_f"+ str(fea)
            if(ran == Ranges[len(Ranges)-1]):
                print(tbl_name+".add_with_SetCode_"+fea_name+"("+fea_name+"_start="+str(ran.split(",")[0])+ \
                    ", "+ fea_name+"_end="+str(65535)+", code0="+str(cods0) + ", code1=" + str(cods1) + ", code2=" + str(cods2) + ")", file = entries_file)
            else:
                print(tbl_name+".add_with_SetCode_"+fea_name+"("+fea_name+"_start="+str(ran.split(",")[0])+ \
                    ", "+ fea_name+"_end="+str(ran.split(",")[1])+", code0="+str(cods0)+", code1="+str(cods1)+", code2=" +str(cods2) + ")", file = entries_file)
